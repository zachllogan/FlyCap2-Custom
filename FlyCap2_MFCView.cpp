...
//	ON_NOTIFY(NM_CLICK, IDC_INFOTREE, &CFlyCap2_MFCView::OnNMClickInfotree)
	...
	ON_COMMAND(ID_TEST_CAPTURE, &CFlyCap2_MFCView::OnTestCapture)
	ON_COMMAND(ID_VIEW_SETUP_SCALE, &CFlyCap2_MFCView::OnViewSetupScale)
	ON_COMMAND(ID_VIEW_DRAWSCALE, &CFlyCap2_MFCView::OnViewDrawscale)
	ON_UPDATE_COMMAND_UI(ID_VIEW_DRAWSCALE, &CFlyCap2_MFCView::OnUpdateViewDrawscale)
	END_MESSAGE_MAP()
...
void CFlyCap2_MFCView::OnDraw(CDC* pDC)
{
	...
	if (ScaleSetupDialog::drawScale)
	{
		ScaleSetupDialog::drawScaleBar(pDoc->m_bitmapInfo.bmiHeader, pImagePixels);
	}
	...
	else
	{
		...
		{
			CImage testImage;
			testImage.Attach(HBITMAP(*dcMem.GetCurrentBitmap()));
			testImage.Save("C:\\Users\\Zach\\Documents\\testerCapture.jpeg", Gdiplus::ImageFormatJPEG);
			testCapture = false;
		}
...
void CFlyCap2_MFCView::OnTestCapture()
{
	testCapture = true;
}

void CFlyCap2_MFCView::OnViewSetupScale()
{
	ScaleSetupDialog sclStpDlg(this);

	INT_PTR nRet = -1;
	nRet = sclStpDlg.DoModal();

	if (nRet == IDOK)
	{
		/*pxlSz = sclStpDlg.pxlSz;
		pxlSzUnt = sclStpDlg.pxlSzUnt;
		drwSclOnLvFd = sclStpDlg.drwSclOnLvFd;
		drwSclOnSvdFl = sclStpDlg.drwSclOnSvdFl;
		mjrTcks = sclStpDlg.mjrTcks;
		mjrTcksSz = sclStpDlg.mjrTcksSz;
		mjrTcksSzUnt = sclStpDlg.mjrTcksSzUnt;
		mjrTcksLbls = sclStpDlg.mjrTcksLbls;
		mjrTcksGrd = sclStpDlg.mjrTcksGrd;
		mnrTcks = sclStpDlg.mnrTcks;
		mnrTcksSz = sclStpDlg.mnrTcksSz;
		mnrTcksSzUnt = sclStpDlg.mnrTcksSzUnt;
		mnrTcksLbls = sclStpDlg.mnrTcksLbls;
		mnrTcksGrd = sclStpDlg.mnrTcksGrd;*/
		/*color = sclStpDlg.color.GetColor();
		magnification = _ttof(sclStpDlg.presetControl.SpanExcluding("X"));*/
	}
}
...
void CFlyCap2_MFCView::OnUpdateViewDrawscale(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(ScaleSetupDialog::drawScale ? TRUE : FALSE);
}
