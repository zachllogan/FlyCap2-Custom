...
ON_COMMAND(ID_COLORPROCESSINGALGORITHM_IPP, &CFlyCap2_MFCDoc::OnColorAlgorithmIPP)
	...
	ON_COMMAND(ID_FILE_LOADMETADATA, &CFlyCap2_MFCDoc::OnFileLoadMetadata)
END_MESSAGE_MAP()
...
void CFlyCap2_MFCDoc::OnFileSaveAs()
{
	...
	if( fileDialog.DoModal() == IDOK )
	{
				...
				(pixelFormat == PIXEL_FORMAT_RAW16))
		{
			...
			{
				error = tempImage.Convert(PIXEL_FORMAT_BGRU, &saveImage);
			}
		}
		...
		if (ScaleSetupDialog::drawScale)
		{
			...
			header.biHeight = saveImage.GetRows();
			header.biWidth = saveImage.GetCols();
			header.biBitCount = saveImage.GetBitsPerPixel();
			ScaleSetupDialog::drawScaleBar(header, saveImage.GetData());
		}
		
		bool saveImg =
		MetaDataSaveLoad::WriteMetaData(reinterpret_cast<FlyCapture2::Camera*>(m_pCamera), fileDialog.GetPathName(), "this is a default test description"
...
bool CFlyCap2_MFCDoc::Start( PGRGuid guid )
{
	...
	//FlyCapture2::Camera* pCam = ;
	//pCam->Connect(&guid);
	FlyCapture2::Format7Info Info;
	FlyCapture2::Format7Info* pInfo = &Info;
	bool tempy;
	bool* temp = &tempy;
	reinterpret_cast<Camera*>(m_pCamera)->GetFormat7Info(pInfo, temp);
	ScaleSetupDialog::loadConfig(m_cameraInfo, *pInfo);
	//MetaDataSaveLoad::ReadMetaData(reinterpret_cast<Camera*>(m_pCamera), "C:\\Users\\Home\\Pictures\\test2.bmp.txt");
...
void CFlyCap2_MFCDoc::OnFileLoadMetadata()
{
	CFileDialog fileDlg(true);
	if (fileDlg.DoModal() == IDOK)
	{
		MetaDataSaveLoad::ReadMetaData(reinterpret_cast<Camera*>(m_pCamera), fileDlg.GetPathName());
