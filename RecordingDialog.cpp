...
Error RecordingDialog::SaveImage( Image* tmp, ImageRecordingPage::ImageSettings* imageSettings, unsigned int count )
{
	...
	if (ScaleSetupDialog::drawScale)
	{
		...
		header.biHeight = tmp->GetRows();
		header.biWidth = tmp->GetCols();
		header.biBitCount = tmp->GetBitsPerPixel();
		ScaleSetupDialog::drawScaleBar(header, tmp->GetData());
	}
...
void RecordingDialog::SaveImageToFile(Image* image, ImageRecordingPage::ImageSettings* imageSettings)
{
			...
			(pixelFormat == PIXEL_FORMAT_RAW16))
	{
		...
		if ((colAlg != NO_COLOR_PROCESSING) && !m_isSaveBayerImageSelected)
		{
			...
			if (image->Convert(PIXEL_FORMAT_BGRU, &convertedImage) == PGRERROR_OK)
			{
...
void RecordingDialog::SaveImageToVideo(AVIRecorder* aviRecorder, Image* image )
{
	...
	{
		BITMAPINFOHEADER header;
		header.biHeight = image->GetRows();
		header.biWidth = image->GetCols();
		header.biBitCount = image->GetBitsPerPixel();
		ScaleSetupDialog::drawScaleBar(header, image->GetData());
	}
